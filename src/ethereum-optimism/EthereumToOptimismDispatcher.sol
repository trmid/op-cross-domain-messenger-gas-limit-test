 ...
 */
contract MessageDispatcherOptimism {
  /* ============ Variables ============ */
  ...
  function dispatchMessageBatch(uint256 _toChainId, MessageLib.Message[] calldata _messages, uint256 _gasLimit)
    external
    returns (bytes32)
  {
    address _executorAddress = _getMessageExecutorAddress(_toChainId);
   ...
   */
  function _sendMessage(address _executor, bytes memory _message, uint256 _gasLimit) internal {
    crossDomainMessenger.sendMessage(_executor, _message, uint32(_gasLimit));
  }
}
